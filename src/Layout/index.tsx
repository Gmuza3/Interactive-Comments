...
const Layout = () => {
  ...
  const innerSubmitButton = (id: number, e: FormEvent, innerId?: number) => {
    e.preventDefault();
    setInputValue("");
    addInInnerReply(inputValue, id);
    if (innerId) {
      setReplyPostId((prev) => ({ ...prev, [innerId]: false }));
    }
  };
  const commentsSubmitButton = (e: FormEvent) => {
    e.preventDefault();
  ...
  return (
    <div className="w-full py-[50px]">
      <ul className="w-full list-none flex flex-col items-center justify-center gap-2">
        {!isLoading &&
          post.comments.map((item, index) => (
            <li
              key={item.id}
              className="max-w-[65%] min-w-[65%]  h-full flex flex-col items-start sm-max:max-w-[85%] ss-max:min-w-[95%] "
            >
              <div className="w-full flex justify-between items-start gap-6 bg-white border-[1px] border-transparent rounded-[8px] p-8 md-max:relative md-max:pb-[75px] md-max:p-4">
                <div
              ...
              sm-max:absolute sm-max:left-4 sm-max:bottom-4 sm-max:flex sm-max:flex-row sm-max:bg-slate-200 sm-max:py-1 sm-max:px-4 sm-max:flex-shrink-0"
                >
                  <span
                    ...
                    onClick={() => increase(item.id, "comments")}
                  >
                    <svg
                  ...
                  <span
                    className="cursor-pointer"
                    onClick={() => decrease(item.id, "comments")}
                  >
                    <svg
                      className="fill-slate-400 hover:fill-[#1e40af]"
                      width="11"
        ...
        <div className="w-[50%] pt-2 max-w-[65%] h-full flex flex-col items-start sm-max:max-w-[85%] ss-max:min-w-[95%]">
          <CurrentUserReplies
            inputValue={inputValue}
            commentsSubmitButton={commentsSubmitButton}
            InputChange={InputChange}
            context="Comments"
            post={post}
          />
